// qs8-rsum: *output += sum(input[i]) over a batch of s8 values.
// Reduction kernel — single int32_t output regardless of input size.
//
...
//
// Output is int32_t per the sol; we store it through a uint32_t buffer so we
// can reuse u32_bit_equal. Bit pattern is what matters for bit-exact compare.
...
// Comparison element count: 1 (single i32 reduction output). The macro
...
static void run_kernel(size_t si, candidate_fn_t fn, uint32_t* out) {
    ...
    *out = 0;
    fn(configs[si][0], input_buf, (int32_t*)out, &rparams);
}
