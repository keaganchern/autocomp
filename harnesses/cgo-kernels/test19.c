// f32-vsub: output[i] = input_a[i] - input_b[i]. Two-config sweep matching
// XNNPACK BinaryElementwiseParameters. `batch` is in BYTES (kernel does
...
static void init_inputs(size_t si) {
    ...
    for (size_t i = 0; i < configs[si][0]; i++) input_b_buf[i] = rand_f1();
}
